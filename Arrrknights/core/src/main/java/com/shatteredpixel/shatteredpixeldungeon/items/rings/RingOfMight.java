...
public class RingOfMight extends Ring {
	...
	public String statsInfo() {
		if (isIdentified()){
			return Messages.get(this, "stats", soloBonus(), new DecimalFormat("#.##").format(100f * (Math.pow(1.045, soloBuffedBonus()) - 1f)));
		} else {
	...
	public static float HTMultiplier( Char target ){
		return (float)Math.pow(1.045, getBuffedBonus(target, Might.class));
	}
